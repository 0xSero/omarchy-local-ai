  ...
  PanelWindow {
    ...
    TestCase {
      ...
      function test_gpu_destination(d) {
        ...
        assertUi(!subject.ui.rows.some(function(r){return r.label.indexOf("Test GPU")===0}),"the GPU page repeats no card row")
        assertUi(/free/.test(subject.ui.sub),"the GPU page names its per-card state in the subtitle")
        subject.snap=Object.assign({},fixtureData,{models:[]})
        subject.activate("gpu:test-gpu"); wait(40); same(subject.view,"card")
        var run=subject.ui.rows.findIndex(function(r){return r.action==="run:model1:1"})
        assertUi(run>=0,"a free model is one row and one action")
        var action=findChild(subject.contentFocus,"content-row-"+run)
        assertUi(action!==null,"the model row exists")
        subject.revealItem(action); wait(30)
        var y=action.mapToItem(viewport,0,0).y
        assertUi(y>=-1 && y+action.height<=viewport.height+1,"the model row is reachable")
        assertUi(!subject.ui.rows.some(function(r){return r.action==="pick:model1:1"}),"no model row needs a selection first")
        subject.snap=JSON.parse(JSON.stringify(fixtureData)); wait(40)
        var blocked=subject.ui.rows.findIndex(function(r){return r.action==="run:model1:1" && /replaces/.test(r.detail||"")})
        assertUi(blocked>=0,"a model that needs occupied GPUs names what it replaces in its own row")
        var reason=findChild(subject.contentFocus,"content-row-"+blocked)
        subject.revealItem(reason); wait(30)
        var reasonY=reason.mapToItem(viewport,0,0).y
        assertUi(reasonY>=-1 && reasonY+reason.height<=viewport.height+1,"the swap row is revealed")
