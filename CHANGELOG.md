...
- A one-card recipe starts on the free card with the most memory free, and `gpu auto` prefers it: on a box whose first card drives the desktop (Hyprland alone holds 3 GB there) a vLLM recipe that asks for 97% of the card could never start while the second card sat empty. A vLLM recipe's `--gpu-memory-utilization` is lowered to the share actually free on the claimed cards, and a start that dies on the KV cache check is retried once at the context vLLM reports, which the model then shows. The engine log kept at rollback is 400 lines, so the root cause is in the plugin's log, not only in a container that is gone.
