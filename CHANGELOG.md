...
- Icons instead of dotted lists: a model's page shows its format, cards, context, vision and size as icon chips; agent (a terminal prompt), folder, this machine and tailnet rows lead with an icon, and weights with the Hugging Face logo; agent and folder open with a chevron and mark the chosen one with a check. A running card reads "Arc Pro B70  32 GB", with speed and tokens as icons in its corner.
