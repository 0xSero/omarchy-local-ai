...
# match_hardware -> {"hardwareId":..,"gpu":{..},"reason":".."}  The first GPU that matches an entry wins;
# an unmatched machine reports why, which is what the panel shows instead of a dead button.
match_hardware() {
  ...
  jq -c --argjson hw "$hw" '
    ...
    [.hardware|to_entries[] as $e | $hw.gpus[] as $g
