...
engine_argv() { # engine_argv <recipe> -> NUL-separated docker argv
  ...
  elif [[ $backend == amd-rocm ]]; then # ROCm needs KFD plus only the PCI-resolved render nodes of claimed cards
    local -a nodes=()
    while IFS= read -r real; do
      [[ $real =~ ^/dev/dri/renderD[0-9]+$ ]] || { fail "no render node found for a selected AMD GPU"; return 1; }
      nodes+=(--device "$real:$real")
    done < <(jq -r '(.gpuRenderNodes // [.gpuRenderNode])[]? // empty' <<<"$r")
    ((${#nodes[@]})) || { fail "no render node found for the selected AMD GPU"; return 1; }
    a+=(--device /dev/kfd:/dev/kfd "${nodes[@]}")
