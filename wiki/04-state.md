...
| `update` | `{enabled, plugin, recipes}` | what the background check staged, and nothing else — `plugin` is `{current, latest}` (`latest` empty when the release in use is newest) and `recipes` is `{generatedAt, commit, new, relevant}`, where the last two count new recipe ids in total and among them the ones for the detected cards |
