...
// falloff. The shimmer is timer-driven over plain Rectangles: they repaint in this shell, where a
...
Item {
  ...
  readonly property int period: p.tone === "work" ? 550 : p.tone === "ready" ? 800 : p.tone === "error" ? 700 : 1100
  readonly property color light: p.toneColor
  property real phase: 0
  Timer { interval: 70; running: p.opened; repeat: true; onTriggered: orb.phase = (orb.phase + 70 / orb.period) % 1 }
  readonly property real litRadius: fullRadius * (0.93 + 0.07 * Math.sin(phase * 2 * Math.PI))
  Repeater {
    ...
    Rectangle {
      ...
      readonly property real lit: {
        ...
        return l * (0.86 + 0.14 * Math.sin((orb.phase + delay) * 2 * Math.PI))
      }
