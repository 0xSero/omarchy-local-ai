...
Panel {
  ...
  readonly property var ui: Model.build({ snap: snap, view: view, hw: hw, count: count, slotSel: slotSel, launcherOpen: launcherOpen, agentPick: agentPick, agentOpen: agentOpen, pending: pending, lastVerb: lastVerb, elapsed: elapsed, localError: localError })
  ...
  onWorkingChanged: if (working) Qt.callLater(home)
  ...
  // one verb at a time; load and unload are done when a snapshot shows their worker, the rest when the process exits
  ...
  function activate(a) {
    if (!a) return
    var s = a.split(":"), v = s[0]
    if (v === "work" || v === "home") home()
    else if (v === "choose-folder") editFolder()
    ...
    else if (v === "launcher-toggle") { launcherOpen = !launcherOpen; agentOpen = false; cursor = 0 }
    else if (v === "agent-toggle") agentOpen = !agentOpen
    else if (v === "agent") { agentPick = s[1]; agentOpen = false; cursor = root.view === "home" ? 2 : 1 }
    else if (v === "open-agent") { if (agentLaunch.running) return; agentLaunch.command = [cli, "agent", s[1], s[2]]; agentLaunch.running = true }
  ...
  Process { id: action; onExited: function(code) { if (root.lastVerb === "agent-dir" && code === 0) { root.editingFolder = false; root.focusContent() } if (["load", "unload"].indexOf(root.lastVerb) < 0) root.pending = false; root.refresh() } }   // a folder that does not exist keeps the editor open
  ...
  onViewChanged: { cursor = 0; editingFolder = false; Qt.callLater(function() { scrollBy(-1e9) }) }
  ...
  KeyboardPanel {
    ...
    Item {
      ...
      Keys.onPressed: function(event) {
        ...
        else if (k === Qt.Key_Escape) { if (root.agentOpen) { root.agentOpen = false; root.cursor = 0 } else if (root.view === "home" && root.launcherOpen) { root.launcherOpen = false; root.cursor = 0 } else if (root.expanded) root.expanded = false; else if (root.view !== "home") root.back(); else root.close() }
        ...
        Column {
          ...
          Column {
            ...
            Controls.TextField {
              ...
              placeholderText: "Project folder path (must exist)"
